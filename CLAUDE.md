...
- **主动发言触发语**：作为用户消息发给 LLM，触发它主动说话。空则用默认（群聊/个人不同）
...
后台线程（每 30~40 分钟 tick，随机扰动 5-10 分钟）:
  ...
  2. 对每个用户:
     ...
     c. 查最新消息时间 → 冷场检查
     d. 冷却检查
     e. LLM 生成（原 system_prompt + 三层记忆 + 触发语）
     f. 发送到微信
```

- 不修改 system_prompt，保持人设一致
- 群聊自动加 `@BOT_NAME` 绕过 `group_at_only`
- 个人私聊不加 `@`，`chat_type=private`
