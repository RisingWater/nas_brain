...
- **图片自动识别** — smart 模式下自动识别图片内容：PaddleOCR 提取文字（置信度过滤），不足 20 字时由多模态 LLM（mimo-v2.5）识别整图，结果存储到聊天历史供后续追问
...
- **群成员备注** — 群聊用户维护「群里谁是谁」的成员名单（sender→外号/职业等备注），注入 LLM 上下文帮助识别消息来源；配置页可从聊天记录一键提取候选成员（按出现次数排序），越聊越全
- **批量消息合并** — 微信群聊多条 @ 消息合并成一次 LLM 调用、一次回复（可选开启，默认关闭），避免各答各的内容重复
- **主动发言（冰点）** — 冷场时 AI 主动说话，支持自定义触发语、沉默时间、冷却、免打扰时段，保持原有人设和记忆，无人回应当天不再打扰
- **用户策略单独页面** — 每个用户的 smart/direct/ignore 策略、工具白名单、System Prompt 以及冰点配置在独立页面编辑
- TTS 缓存 — 重复文本跳过合成
- 服务管理器 — Web 上启停各微服务
- 微信聊天记录搜索
...
- **监控面板** (`/`) — 资源使用概览（CPU/内存/存储饼图 + 请求/Token/耗时柱状图）
- **请求追踪** (`/traces`) — 全链路耗时分析（语音唤醒→STT→LLM→TTS→播放）
- **数据备份** (`/backup`) — data 目录打包、下载、删除
- **AI 状态表情** (`/ai-status`) — 实时显示 AI 当前状态的表情界面

### AI 状态表情

独立全屏页面 `/ai-status`，通过 SVG 表情实时反映 AI 状态：

| 状态 | 颜色 | 表情 | 说明 |
|------|------|------|------|
| 空闲 | 青蓝 | 正常眼 + 平眉 | 等待输入，偶有眨眼和挑眉 |
| 聆听 | 橙色 | 大圆杏眼 + 微抬眉 | 双脉冲圈 47→37 收缩淡出 |
| 思考 | 橙黄 | 大眼旋转 + 锁眉 | 三圆弧旋转 loading |
| 操作 | 珊瑚 | 方形锐眼 + 下压眉 | 三圆弧旋转 loading |
| 说话 | 浅绿 | 弯月眼 + 舒展眉 | 双脉冲圈 37→47 扩散淡入 |

所有状态共享右眉毛随机挑动动画。支持 `?debug=1` 参数手动切换状态预览表情。
...
群聊用户可在配置页维护「群成员备注」（成员 sender→备注 映射，外号/职业等），注入 LLM 上下文帮助识别消息来源；「从聊天记录提取」区可一键添加候选成员（实时统计未记录说话人，按出现次数排序）。
